...
MapboxGL.setAccessToken(
	'pk.eyJ1Ijoid2lubml1YSIsImEiOiJjbDBrbWFhdTEwbTczM2Nwd21nODdzZGhpIn0.7j_9ryv199IqfNbkcGSlrw',
);
...
export default CollectionPoints = ({ route, navigation }) => {
	...
	const fetchCollectionPoints = () => {
		...
		setDataLoaded(false);
		sampleTracking
			.getAllCollectionPoints(Util.getFilteredDate(), navigation)
			.then(data => {
	...
	const getSpecifiedData = value => {
		...
		let filterdata = collectionPointList
			.filter(item => item._id == value.properties.id)
			.map(({ pointId, name, location, type }) => ({
				pointId,
				name,
				location,
				type,
	...
	const renderAnnotation = counter => {
		...
		const co_ordinate = [
			collectionPoint.location.coordinates[0],
			collectionPoint.location.coordinates[1],
		];
