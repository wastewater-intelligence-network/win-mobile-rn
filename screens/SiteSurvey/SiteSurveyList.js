...
const SiteSurveyList = ({ navigation }) => {
	...
	const fetchSchedules = () => {
		...
		setDataLoaded(false);
		sampleTracking
			.getAllPointsSurveyList(Util.getFilteredDate(), navigation)
			.then(data => {
	...
	const siteSurveyHandle = id => {
		let filterdata = scheduleList
			.filter(item => item._id == id)
			.map(({ pointId, name, location, type, samplingType }) => ({
				pointId,
				name,
				location,
				type,
		...
		sampleTracking
			.upgradeSurveyPointToCollectionPoint(
				...
				jsonData.pointId,
				jsonData.type,
				jsonData.samplingType,
				navigation,
			)
			.then(data => {
				ToastAndroid.showWithGravity(
					Constants.alertMessages.collectionAddedSuccessfully,
					ToastAndroid.SHORT,
					ToastAndroid.BOTTOM,
				);
